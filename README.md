...
| 数据查询 | 女武神、便笺、深渊、战场、往世乐土查询；支持@他人查询已绑定UID数据 |
...
| 命令 | 说明 |
|------|------|
| `bbb查询` | 查询女武神首页/概况；可@他人查对方数据（需开启@他人查询配置） |
| `bbb便笺` | 查询实时便笺（体力/日程）；可@他人查对方数据（需开启@他人查询配置） |
| `bbb深渊` | 查询深渊/超弦空间战报；可@他人查对方数据（需开启@他人查询配置） |
| `bbb战场` | 查询战场战报/记忆战场；可@他人查对方数据（需开启@他人查询配置） |
| `bbb往世乐土` | 查询往世乐土记录；可@他人查对方数据（需开启@他人查询配置） |
| `bbb刷新面板` | 刷新角色数据缓存；可@他人查对方数据（需开启@他人查询配置） |
...
| 命令 | 说明 |
|------|------|
| `bbb乐土攻略 <关键词>` | 查询往世乐土攻略图 |
| `bbb攻略 <关键词>` | 查询往世乐土攻略图简写 |
| `bbb<关键词>乐土攻略` | 自然格式查询攻略图 |
| `bbb更新乐土攻略` | 手动刷新攻略索引（默认管理员可用） |
| `bbb乐土关键词列表 [关键词]` | 查看或筛选可用攻略关键词 |
...
默认情况下，@他人查询处于关闭状态；如需使用，请在配置中开启 `@他人查询（BBBAllowAtQuery）`。
如果未开启 `@他人查询（BBBAllowAtQuery）` 配置，则会提示该功能未开放；如果被@用户未绑定UID，会提示对方未绑定。
...
往世乐土攻略数据来源于 [MskTmi/ElysianRealm-Data](https://github.com/MskTmi/ElysianRealm-Data)。该数据源使用 AGPL-3.0，攻略图源来自崩坏3通讯中心（月光中心），仅供交流学习使用。

BBBUID 不内置该仓库图片资源，仅在用户查询时按需拉取并缓存到本地 GsCore 资源目录。
